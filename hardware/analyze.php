...
// Save user system
$stmt = $conn->prepare(
    "INSERT INTO user_systems (user_id, cpu_id, gpu_id, ram_amount, game_id) 
     VALUES (?, ?, ?, ?, ?)"
);
...
// Load user hardware
$result = $conn->query(
    "SELECT c.name AS cpu, c.overall_performance, c.gaming_performance, c.tier AS cpu_tier,
            g.name AS gpu, g.relative_performance, g.tier AS gpu_tier,
            u.ram_amount, u.ram_performance
     FROM user_systems u
...
$gameQuery = $conn->prepare(
    "SELECT gm.name, gm.recommended_ram,
            rc.name AS required_cpu, rc.gaming_performance AS required_cpu_score,
            rg.name AS required_gpu, rg.relative_performance AS required_gpu_score
     FROM games gm
     JOIN cpus rc ON gm.recommended_cpu_id = rc.id
     JOIN gpus rg ON gm.recommended_gpu_id = rg.id
     WHERE gm.id = ?"
);
...
echo "<h3>Selected Game:</h3>" . htmlspecialchars($game['name']) . "<br><br>";
