...
$gameQuery = $conn->prepare( "SELECT gm.name, gm.recommended_ram, rc.name AS required_cpu, rc.gaming_performance AS required_cpu_score, rg.name AS required_gpu, rg.relative_performance AS required_gpu_scoreFROM games gm   JOIN cpus rc ON gm.recommended_cpu_id = rc.idJOIN gpus rg ON gm.recommended_gpu_id = rg.idWHERE gm.id = ?");
